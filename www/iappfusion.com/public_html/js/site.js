...
(function($){
	...
	$.fn.contactable = function(options) {
		...
		return this.each(function(options) {
			...
			<p id="callback"></p>\
			<p>\
			<label for="ContactName">Name <span class="red"> * </span></label>\
			<br />\
			<input id="ContactName" class="contact" name="name" /></p>\
			<p>\
			<label for="ContactEmail">E-Mail <span class="red"> * </span></label>\
			<br />\
			<input id="ContactEmail" class="contact" name="email" />\
			</p>\
			<p>\
			<label for="ContactInterest">I\'m interested in</label>\
			<select name="data[Contact][interest]" id="ContactInterest">\
				<option value="Company">iAppFusion</option>\
				<option value="ConsumerApps">Consumer Apps</option>\
				<option value="BusApps">Business Apps</option>\
			...
			<p>\
			<label for="ContactMessage">Message <span class="red"> * </span></label>\
			<br />\
			<textarea id="ContactMessage" name="comment" class="comment" rows="4" cols="30" ></textarea>\
			</p>\
			<p>\
			...
			$("#contactForm").validate({
				...
				submitHandler: function() {
					$('.holder').hide();
					$('#loading').show();
					var webMethod = 'mailservice.asmx/SendMail';
    				var name = $("#ContactName").val();
    				var email = $("#ContactEmail").val();
    				var phone = $("#ContactPhone").val();
    				var contactInterest = $('#ContactInterest :selected').text();
    				var message = $("#ContactMessage").val();
					$.post('/site/mail.php', $("#contactForm").serialize(),
					function(data){
...
$(function(){
	...
	if($(document.body).height() < $(window).height())
	{
		...
		$("#sticky-footer-push").height(diff - 15);
	}


	var loc = window.location;
	var title = document.title;
	$('#header').append('<div id="social_nav_horizontal"><ul><li><a class="facebook" href="http://www.facebook.com/sharer.php?u='+loc+'&t='+title+'"  title="Find us on Facebook" target="_new"></a></li><li><a class="twitter" href="http://twitter.com/iAppFusion" title="Follow Us On Twitter"  target="_new"></a></li></ul></div>');
